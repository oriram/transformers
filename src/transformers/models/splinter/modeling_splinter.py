# coding=utf-8
# Copyright 2021 Ori Ram, Yuval Kirstain, Jonathan Berant, Amir Globerson, Omer Levy The HuggingFace Inc. team. All rights reserved.
#
...
def load_tf_weights_in_splinter(model, config, tf_checkpoint_path):
...
class SplinterPredictionHeadTransform(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.dense = nn.Linear(config.hidden_size, config.hidden_size)
        if isinstance(config.hidden_act, str):
...
class SplinterLMPredictionHead(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.transform = SplinterPredictionHeadTransform(config)
...
class SplinterOnlyMLMHead(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.predictions = SplinterLMPredictionHead(config)
...
@add_start_docstrings("""Splinter Model with a `language modeling` head on top. """, SPLINTER_START_DOCSTRING)
class SplinterForMaskedLM(SplinterPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)

        if config.is_decoder:
            logger.warning(
                "If you want to use `SplinterForMaskedLM` make sure `config.is_decoder=False` for "
                "bi-directional self-attention."
            )

        self.splinter = SplinterModel(config)
        self.cls = SplinterOnlyMLMHead(config)
...
@add_start_docstrings(
    """Splinter Model with a `language modeling` head on top for CLM fine-tuning. """, SPLINTER_START_DOCSTRING
)
class SplinterForCausalLM(SplinterPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if not config.is_decoder:
            logger.warning("If you want to use `SplinterForCausalLM` as a standalone, add `is_decoder=True.`")
...
class SplinterForSequenceClassification(SplinterPreTrainedModel):
    def __init__(self, config):
        ...
        self.splinter = SplinterModel(config)
        self.classifier = SplinterClassificationHead(config)
